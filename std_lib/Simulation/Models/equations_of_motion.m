function rocket = equations_of_motion(rocket)
    ...
    function body = assign_body_parameters_if_unassigned(body)
        ...
        if ~isfield(body, 'forces'                    ); body.forces                      = struct('Null',force( [0;0;0],[0;0;0])); end
        if ~isfield(body, 'moments'                   ); body.moments                     = struct('Null',moment([0;0;0],[0;0;0])); end
        ...
        if isfield(body, "center_of_mass"); body.center_of_mass_absolute = body.center_of_mass_absolute;
        else;                               body.center_of_mass_absolute = body.position;
        end
        body.mass_absolute           = body.mass;
        body.force_absolute          = force (cellsum(cellfun( @(force)       body.forces .(force ).vec                              , fieldnames(body.forces ), 'UniformOutput', false)), [0;0;0]);
        body.moment_absolute         = moment(cellsum(cellfun( @(moment)      body.moments.(moment).vec                              , fieldnames(body.moments), 'UniformOutput', false)) + ...
                                              cellsum(cellfun( @(force) cross(body.forces .(force ).pos, body.forces.(force ).vec) ,   fieldnames(body.forces ), 'UniformOutput', false)), [0;0;0]);
